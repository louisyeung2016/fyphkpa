...
function showQRCode(text) {
  ...
  var dotsize = document.getElementById("dotsize").value;  // size of box drawn on canvas
  var padding = 10; // (white area around your QRCode)
  var black = "rgb(0,0,0)";
  var white = "rgb(255,255,255)";
  var QRCodeVersion = document.getElementById("QRCodeVersion").value; // 1-40 see http://www.denso-wave.com/qrcode/qrgene2-e.html
	
	var ECL = document.getElementById("ECL").value;
	var n = Number(ECL); //convert Object to number datatype
  ...
  try {
    ...
    var qr = new QRCode(QRCodeVersion, n); 
   	qr.addData(text);
   	qr.make();
   }
  catch(err) {
