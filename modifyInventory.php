...
include_once("./language/modifyInventory_string.php");
...
/*
$sql1 = "SELECT * FROM `item_stock_record` ORDER BY stock_record_id DESC";
		...
		<details open>
			  ...
			  <br>
				...
				<select name="searchBy" id="searchBy" >
					<option value="itemid"><?php echo $table_Title1[$lang]; ?></option>
					<option value="itemname"><?php echo $table_Title2[$lang]; ?></option>
					<option value="qty"><?php echo $table_Title3[$lang]; ?></option>
					<option value="compensation"><?php echo $table_Title4[$lang]; ?></option>
					<option value="qrcode"><?php echo $table_Title5[$lang]; ?></option>
							  ...
							  <tbody>
								...
								if(mysqli_num_rows($rs) > 0){
									do { 
										...
										echo " <tr><td> $itemID </td><td> $itemName </td><td> $inventoryQty </td><td> $compensationPrice </td><td> $qrCode </td><td> $itemStatus </td>";
...
<script>
	...
	function showItem(str) {
	  if (str == "") {
		...
		xmlhttp.open("GET","getinventory.php?q1="+str,true);
		xmlhttp.send();
	  }
	}
	...
	function showItem2(str) {
	  if (str == "") {
		...
		xmlhttp.open("GET","getinventory.php?q2="+str,true);
		xmlhttp.send();
	  }
	}
	...
	function getStockRecord(){
		...
		$.ajax({
			...
			data:{itemStockRequest:inputMonth}, 
			...
			success: function(result, status, xhr){
				if(result == 0){
					...
					$.each(result_obj, function (i, item1) {
						...
						var trHTML = '';
						trHTML += '<tr><td>' + result_obj[i][0] + '</td><td>'+ result_obj[i][1] + '</td><td>' + result_obj[i][2] + '</td><td>' + result_obj[i][3] + '</td><td>' + result_obj[i][5] + '</td><td>' + result_obj[i][6] + '</td><td>' + result_obj[i][7] + '</td><td>' + result_obj[i][4] + '</td></tr>';
